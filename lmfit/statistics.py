...
"""Provide the implementations of Statistic object."""
...
class CStatistic(Statistic):
    ...
    def objective_func(self, model, dependent_var):
        ...
        if np.sum(model < 0):
            warnings.warn("Negative values in model.")
...
class ChiSquareStatistic(Statistic):
    ...
    def __init__(self, optimization_method='leastsq',
        ...
        if self.variance_approximation is not None:
            name = '{} with {} variance_approximation'.format(name,
    ...
    def objective_func(self, model, dependent_var, weights=None):
        """Objective function minimized in fit.
    ...
    def _weight_input(self, model, dependent_var, weights=None):
        if self.variance_approximation == 'Neyman':
            weights = self.weights_from_variance(dependent_var)
        elif self.variance_approximation == 'Pearson':
            weights = self.weights_from_variance(model)
        return (self.apply_weight(model, weights),
                self.apply_weight(dependent_var, weights))

    @staticmethod
    def weights_from_variance(array):
...
class ChiSquareStatisticVariableProjection(ChiSquareStatistic):
    ...
    variance : ndarray, optional
        None by default. Variance for weighted least squares.
    variance_approximation : str, optional
    ...
    def objective_func(self, model, dependent_var, weights=None):
        ...
        Calculates weighted residuals as variable projection of `dependent_var`.
